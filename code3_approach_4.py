...
def select_retrieval_heads(train_queries, model, tokenizer, tools, device, max_heads=20):
    ...
    for qix in tqdm(range(len(train_queries))):
        ...
        #    for layer_idx, layer_attn in enumerate(attentions):
            ...
            # Score each tool for each head
            num_tools = len(item_spans)
            tool_scores = torch.zeros(num_heads, num_tools, device=device)
            
            for tool_id, span in enumerate(item_spans):
                tool_scores[:, tool_id] = query_attn[:, :, span[0]:span[1]].sum(dim=(1, 2))
            
            gold_tool_id = map_docname_id[gold_tool_name]
            
            # Rank tools for each head; add 1 if gold is rank-1, 0 otherwise
            ranked = torch.argsort(tool_scores, dim=1, descending=True)
            gold_ranks = (ranked == gold_tool_id).nonzero(as_tuple=False)
            
            for head_idx in range(num_heads):
                gold_rank_for_head = (ranked[head_idx] == gold_tool_id).nonzero()[0].item()
                head_scores[layer_idx, head_idx] += 1.0 / (gold_rank_for_head + 1)  # MRR-style
